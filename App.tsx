...
const App: React.FC = () => {
  ...
  const handleRecommend = async () => {
    ...
    try {
      ...
      setState(prev => ({ ...prev, loading: false, error: '레시피를 가져오는 중 오류가 발생했습니다.' }));
    }
  };
  ...
  const handleSaveToDrive = () => {
    ...
    const client = google.accounts.oauth2.initTokenClient({
      client_id: 'YOUR_CLIENT_ID.apps.googleusercontent.com', // 실제 환경에서는 Client ID가 필요합니다.
  ...
  return (
    <div className="min-h-screen py-10 px-4 max-w-3xl mx-auto">
        ...
        <div className="p-8">
          <section className="mb-10">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <input
                ...
                className="flex-1 p-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-orange-400 focus:ring-0 transition-all text-lg"
              />
              <button
                onClick={handleRecommend}
                disabled={state.loading}
                className="px-8 py-5 rounded-2xl font-black text-lg text-white bg-gradient-to-r from-orange-500 to-red-500 hover:scale-105 transition-transform disabled:opacity-50"
              >
          ...
          <section className="space-y-10">
            ...
            <div className="mt-12">
              ...
              <div className="group relative w-full aspect-video rounded-3xl overflow-hidden bg-gray-50 border-4 border-dashed border-gray-200 flex items-center justify-center">
                {state.image ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-4">
                      <button 
                        ...
                        className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-2xl font-black text-lg shadow-2xl flex items-center gap-3 transform hover:scale-105 transition-all"
                      >
                        {uploadStatus === 'uploading' ? '업로드 중...' : '구글 드라이브 폴더에 직접 저장'}
                      ...
                      <a href={STORAGE_FOLDER_URL} target="_blank" className="text-white underline text-sm">저장 폴더 확인하기</a>
                    </div>
                  </>
                ) : (
                  <p className="text-gray-400 font-medium">이미지 생성 대기 중...</p>
                )}

                {uploadStatus === 'done' && (
                  <div className="absolute inset-0 bg-green-600/90 flex flex-col items-center justify-center text-white p-6 animate-fadeIn">
                    <span className="text-5xl mb-4">✅</span>
                    <h3 className="text-2xl font-black mb-2">저장 완료!</h3>
                    <p className="text-center opacity-90 mb-4">지정하신 구글 드라이브 폴더에<br/>이미지가 성공적으로 전송되었습니다.</p>
                    <button onClick={() => setUploadStatus('idle')} className="bg-white text-green-700 px-6 py-2 rounded-full font-bold">확인</button>
                  </div>
                )}
              </div>
            </div>
      ...
      <footer className="mt-16 text-center">
        <img src={footerLogoUrl} alt="Logo" className="w-16 h-16 rounded-full mx-auto mb-4 border-2 border-orange-200" />
